...
def preprocess(df: pd.DataFrame) -> tuple:
    ...
    if "churn" not in df.columns:
        ...
        target_candidates = [
            ...
            if "churn" in c.lower() or "exited" in c.lower()
        ]

        if target_candidates:
            df = df.rename(columns={target_candidates[0]: "churn"})
            st.info(f"ℹ️ Renamed column `{target_candidates[0]}` → `churn`.")
        else:
            raise ValueError(
                "No target column found. Expected something like 'churn' or 'exited'. "
