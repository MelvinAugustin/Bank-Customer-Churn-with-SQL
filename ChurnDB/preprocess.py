...
def preprocess(df: pd.DataFrame) -> tuple:
    ...
    CHURN_ALIASES = {"churn", "exited", "churned", "target", "label"}
    if "churn" not in df.columns:
        churn_candidates = [c for c in df.columns if c in CHURN_ALIASES or "churn" in c or "exit" in c]
        if churn_candidates:
            df = df.rename(columns={churn_candidates[0]: "churn"})
        else:
            raise ValueError(
                f"No churn target column found (looked for: {sorted(CHURN_ALIASES)}). "
